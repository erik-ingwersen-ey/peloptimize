...
dados = {
    "Processo": df.index,
    #         'bentonita' : ((df['PESO1_I@08MI-LW-832I-01M1'] + df[
    #         'PESO1_I@08MI-LW-832I-02M1']) / (df[
    #         'PESO1_I@08MI-BW-832I-01M1'] + df['PESO1_I@08MI-BW-832I-02M1'])
    #         * 100 / 0.8693) / df['PROD_PQ_Y@08US'],
    # calc atualizado
    "bentonita": (
        df["PESO1_I@08MI-LW-832I-01M1"] + df["PESO1_I@08MI-LW-832I-02M1"]
    )
    / df["PROD_PQ_Y@08US"]
    * 1000,
    ...
    #         'rel_pressaoArAsc_Desc': df['PRES1_C@08QU-PF-852I-06M1'] / df[
    #         'PRES1_C@08QU-VD-853I-17M1'],
    "Soma_FUNC1_D_PE-BD-840I": df[
        [x for x in df.columns if "FUNC1_D@08PE-BD-840I-" in x]
    ].sum(
        1
    ),  #
    "Soma_PESO1_I_PE-BW-840I-01_12": df[
        [x for x in df.columns if "PESO1_I@08PE-BW-840I-" in x]
    ].sum(1),
    "Media_NIVE1_6_QU-FR-851I-01M1": df[
        [x for x in df.columns if "NIVE" in x and "@08QU-FR-851I-" in x]
    ]
    .iloc[:, :-1]
    .mean(1),
    "Std_NIVE1_6_QU-FR-851I-01M1": df[
        [x for x in df.columns if "NIVE" in x and "@08QU-FR-851I-" in x]
    ]
    .iloc[:, :-1]
    .std(1),
    "NIVE7_I@08QU-FR-851I-01M1": df["NIVE7_I@08QU-FR-851I-01M1"],
    "Media_ROTA1_I_PE-BD-840I-01_12": df[
        [x for x in df.columns if "ROTA1_I@08PE-BD-840I-" in x]
    ].sum(1)
    / df[[x for x in df.columns if "FUNC1_D@08PE-BD-840I-" in x]].sum(1),
    "Std_ROTA1_I_PE-BD-840I-01_12": df[
        [x for x in df.columns if "ROTA1_I@08PE-BD-840I-" in x]
    ].std(1),
    "UMID_H2O_PR_L@08FI": df["UMID_H2O_PR_L@08FI"],
    "VAZA1_I@08QU-ST-855I-01": df["VAZA1_I@08QU-ST-855I-01"],
    ## Adicionado com base no dicionário de dados.
    "FUNC2_D@08PR-TR-822I-01M1": df["FUNC2_D@08PR-TR-822I-01M1"],
    "ROTA1_I@08QU-PF-852I-01M1": df["ROTA1_I@08QU-PF-852I-01M1"],
    # ventilador resfriamento 52S26
    "ROTA1_I@08QU-PF-852I-07M1": df["ROTA1_I@08QU-PF-852I-07M1"],
    # ventilador resfriamento 52S31
...
df2 = pd.concat(
    [df2, df[[x for x in df.columns if "TEMP1_I@08QU-QU-855I-GQ" in x]]], axis=1
)

df2 = pd.concat(
    [df2, df[[x for x in df.columns if "TEMP5_I@08QU-PF-852I-" in x]]], axis=1
)

df2 = pd.concat(
    [df2, df[[x for x in df.columns if "GRAN_OCS_TM@08PE-BD-840I-" in x]]],
    axis=1,
)
df2 = pd.concat(
    [df2, df[[x for x in df.columns if "GRAN_OCS_10-12@08PE-BD-840I-" in x]]],
    axis=1,
)
df2 = pd.concat(
    [df2, df[[x for x in df.columns if "GRAN_OCS_12-16@08PE-BD-840I-" in x]]],
    axis=1,
)

for num in np.arange(1, 13):
    df2["GRAN_OCS_others_" + str(num).zfill(2)] = df[
        [
            x
            for x in df.columns
            if ("GRAN_OCS" in x)
            and ("PE-BD-840I-" + str(num).zfill(2) in x)
            and not ("10-12" in x)
            and not ("12-16" in x)
            and "GRAN_OCS_TM" not in x
        ]
    ].sum(axis=1)
...
for col in cols:
    df2[col] = pd.to_numeric(df2[col], errors="coerce").fillna(
        value=0
    ) / pd.to_numeric(df2["PROD_PQ_Y@08US"], errors="coerce").fillna(value=0)
...
limits = {
    ...
    "PRES1_OCS_S@08QU-PF-852I-04M1": (
        df2["PRES1_OCS_S@08QU-PF-852I-04M1"].quantile(0.003),
        df2["PRES1_OCS_S@08QU-PF-852I-04M1"].quantile(0.992),
    ),
    "PRES1_C@08QU-PF-852I-06M1": (
        df2["PRES1_C@08QU-PF-852I-06M1"].quantile(0.002),
        -4,
    ),
    "PRES1_C@08QU-VD-853I-17M1": (
        df2["PRES1_C@08QU-VD-853I-17M1"].quantile(0.003),
        df2["PRES1_C@08QU-VD-853I-17M1"].quantile(0.995),
    ),
    "GRAN_-0,045_PR_L@08FI": (
        df2["GRAN_-0,045_PR_L@08FI"].quantile(0.003),
        df2["GRAN_-0,045_PR_L@08FI"].quantile(0.999),
    ),
    ...
    #      **{t: (df2[t].quantile(0.12), None) for t in df2[[x for x in
    #      df.columns if 'GRAN_OCS_TM@08PE-BD-840I-' in x]]}
}
...
# # print('porcentagem do total da base: {}'.format(n_outliers / df2.shape[0]
# * 100))
...
for t in df_train.columns:
    ...
    df_train[t] = (
        ...
        .interpolate()
        .fillna(method="bfill")
        .rolling(media_movel, min_periods=1)
        .mean()
    )
...
df_train = df_train[[c for c in df_train if c != col_target] + [col_target]]
...
#     results += apply_model(method_conf, col_target, method, df_train,
#     df_target)
...
#         # Métricas utilizadas
#         columns=['Modelo', 'Params', 'MSE','MAPE', 'R2', 'R', 'R2 Train',
#         'R2 Train Adj'])
