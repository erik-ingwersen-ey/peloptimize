""" Operations applied over the the data """
import numpy as np
import os
import json
import re
...
class operations:
  ...
  @staticmethod
  def unnormalize_feature(scalers, feature,
                          norm_value, operation='two feature'):

      scale_var = scalers[feature]
      if operation == 'two feature':
          # old normalize var
          first_value = norm_value/scale_var.data_range_[0]
          second_value = norm_value * scale_var.data_min_[0]/scale_var.data_range_[0]
  ...
  def retrieve_dataset(feature, datasets):
      ...
      return list(filter(lambda col: feature in datasets[col].columns,
                      datasets.keys()))
  ...
  def define_real_scalers(datasets, df_map=None):
    ...
    all_columns = list(filter(lambda x: 'qtde' not in x and 'NIVE1' not in x and 'SOMA_FUNC' not in x, all_columns))
...
#         if (column in df_map and df_map[column] in constants.TARGETS_IN_MODEL.keys()) or column in constants.TARGETS_IN_MODEL.keys():


#             if column in df_map:

#                 new_column = constants.TARGETS_IN_MODEL[df_map[column]]

#                 scalers[new_column] = scalers[column]

#                 del scalers[column]

#             else:

#                 new_column = constants.TARGETS_IN_MODEL[column]
...
#       scalers = add_plant_new_keys(scalers, df_map)
  ...
  def define_integer_scalers(df_all, scalers):
    for col in df_all.columns:
      if col not in scalers and (col.startswith('qtde') or col.startswith('SOMA FUNC')):
