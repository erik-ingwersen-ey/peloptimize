...
#  Usar numero menor caso queira testar código.
n_dias = 547  # padrão: 547 dias.
n_dias2 = 730  # padrão: 730 dias

# MAGIC %md
# MAGIC # carrega variáveis e funções
...
import wip.Otimizacao.modules.predictive_module
import pelopt.utils.model_module
import pelopt.utils.utils
import pelopt.utils.sql_module
import wip.Otimizacao.files.process_tag_map
import wip.Otimizacao.files.datasets_limits
import wip.Otimizacao.files.datasets_confs

# MAGIC %md
# MAGIC * sql_module: Executando o notebook sql_module para conectar ao banco
# de dados, realizar consultas no Data Lake e no banco de dados
...
faixamin = 700
faixamax = 1000
df_faixa = pd.DataFrame({'faixaMin': list(range(faixamin, faixamax, 50)),
                         'faixaMax': list(
                             range(faixamin + 50, faixamax + 50, 50))})
...
df_sql = sqlModule(dbutils).getFVarParam(n_usina, tini_str, tfim, taglist,
                                         param="US8")
...
# Realiza pivot (tabelas dinâmicas) que permitem resumir a base de dados e
# analisar pelas colunas data, variavel e valor
...
# df_sql.isna().sum().reset_index().sort_values(by = [0],
...
df_sql = sqlModule(dbutils, spark).getTAGsDelay(df_sql, tini, tfim, tagpims,
                                                taglist)
...
# add gabi
# Pressão de vácuo
for t in df_sql.loc[:, ['PRES1_I@08FI-FL-827I-01', 'PRES1_I@08FI-FL-827I-02',
                        'PRES1_I@08FI-FL-827I-03', 'PRES1_I@08FI-FL-827I-04',
                        'PRES1_I@08FI-FL-827I-05R', 'PRES1_I@08FI-FL-827I-06',
                        'PRES1_I@08FI-FL-827I-07', 'PRES1_I@08FI-FL-827I-08',
                        'PRES1_I@08FI-FL-827I-09', 'PRES1_I@08FI-FL-827I-10R']]:
...
for t in df_sql.loc[:,
         [x for x in df_sql.columns if 'ROTA1_I@08FI-FL-827I-' in x]]:
...
# MAGIC %md
# MAGIC #calculadas
# MAGIC * Tags que precisam realizar cálculo para atender as necessidades da
# usina


# 1
# df_sql['CONS ESP VENT TOTAL - US8'] = df_sql['CONS1_Y@08QU-PF-852I-01M1'] +
# df_sql['CONS1_Y@08QU-PF-852I-02M1'] + df_sql['CONS1_Y@08QU-PF-852I-03M1'] +
# df_sql['CONS1_Y@08QU-PF-852I-04M1'] + df_sql['CONS1_Y@08QU-PF-852I-05M1'] +
# df_sql['CONS1_Y@08QU-PF-852I-06M1'] + df_sql['CONS1_Y@08QU-PF-852I-07M1'] +
# df_sql['CONS1_Y@08QU-PF-852I-08M1']
df_sql['CONS ESP VENT TOTAL - US8'] = df_sql['CONS1_Y@08QU-VENT']

# 2
# p = df_sql['PESO1_I@08MO-BW-821I-03M1'] > 0.1
# df_sql['Consumo de Energia (base minério úmido) kWh/ton 3'] = (df_sql[
# 'POTE1_I@08MO-MO-821I-03M1'][p]/df_sql['PESO1_I@08MO-BW-821I-03M1'][p])*1000

# 3
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV27 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-27"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 49 - é pre requisito para outra coluna
df_sql["Vel Periferica R Fixo [m/s]"] = (
    math.pi * df_sql["ROTA1_I@08PR-RP-822I-01M1"] * (2 / 57.07)
) / 60

# 4
p = df_sql["Vel Periferica R Fixo [m/s]"] > 0
df_sql["PRODUTI MDOT PRENSA - US8"] = df_sql["PESO1_I@08PR-BW-822I-01M1"][p] / (
    2 * 1.5 * df_sql["Vel Periferica R Fixo [m/s]"][p]
)

# 5
# df_sql['DESV MEDIO ALT CAMADA'] = mad([df_sql['NIVE1_C@08QU-FR-851I-01M1'],
# df_sql['NIVE2_I@08QU-FR-851I-01M1'], df_sql['NIVE3_I@08QU-FR-851I-01M1'],
# df_sql['NIVE4_I@08QU-FR-851I-01M1'], df_sql['NIVE5_I@08QU-FR-851I-01M1'],
# df_sql['NIVE6_I@08QU-FR-851I-01M1']], axis=0)
...
# 6
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV18 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-18"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 7
df_sql["PV TEMP GQ3-16-MED - US8"] = np.mean(
    [
        df_sql["TEMP1_I@08QU-QU-855I-GQ03"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ04"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ05"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ06"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ06"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ07"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ08"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ09"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ10"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ11"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ12"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ13"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ14"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ15"],
        df_sql["TEMP1_I@08QU-QU-855I-GQ16"],
    ],
    axis=0,
)

# 8
df_sql["PROD FILTR - US8"] = (
    df_sql["PESO1_I@08FI-TR-827I-01M1"] + df_sql["PESO1_I@08FI-TR-827I-02M1"]
)

# 9
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV19 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-19"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 57 (pré-requisito para o #10)
df_sql["Nº FILTR FUNC - US8"] = (
    df_sql["FUNC1_D@08FI-FL-827I-01"]
    + df_sql["FUNC1_D@08FI-FL-827I-02"]
    + df_sql["FUNC1_D@08FI-FL-827I-03"]
    + df_sql["FUNC1_D@08FI-FL-827I-04"]
    + df_sql["FUNC1_D@08FI-FL-827I-05R"]
    + df_sql["FUNC1_D@08FI-FL-827I-06"]
    + df_sql["FUNC1_D@08FI-FL-827I-07"]
    + df_sql["FUNC1_D@08FI-FL-827I-08"]
    + df_sql["FUNC1_D@08FI-FL-827I-09"]
    + df_sql["FUNC1_D@08FI-FL-827I-10R"]
)

df_sql["Nº BV FUNC - US8"] = (
    df_sql["FUNC1_D@08FI-BV-827I-01M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-02M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-03M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-04M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-05RM1"]
    + df_sql["FUNC1_D@08FI-BV-827I-06M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-07M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-08M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-09M1"]
    + df_sql["FUNC1_D@08FI-BV-827I-10RM1"]
)

# 10
# df_sql['Relação bombas x filtros'] = df_sql['Nº BV FUNC - US8']/df_sql['Nº
# FILTR FUNC - US8']

# 11
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV3A - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-03A"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 12
df_sql["Vel Periferica R Móvel [m/s]"] = (
    math.pi * df_sql["ROTA1_I@08PR-RP-822I-01M2"] * (2 / 57.07)
) / 60

# 13
df_sql["Paralelismo"] = np.abs(
    df_sql["POSI1_I@08PR-RP-822I-01"] - df_sql["POSI2_I@08PR-RP-822I-01"]
)

# 14
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV15 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-15"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 15
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV3B - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-03B"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 16
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV2 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-02"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 17
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV12 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-12"] / df_sql["PROD_PQ_Y@08US"][p]
)

# 18
df_sql["MAIOR - MENOR ALT CAMADA"] = np.max(
    [
        df_sql["NIVE1_C@08QU-FR-851I-01M1"],
        df_sql["NIVE2_I@08QU-FR-851I-01M1"],
        df_sql["NIVE3_I@08QU-FR-851I-01M1"],
        df_sql["NIVE4_I@08QU-FR-851I-01M1"],
        df_sql["NIVE5_I@08QU-FR-851I-01M1"],
        df_sql["NIVE6_I@08QU-FR-851I-01M1"],
    ],
    axis=0,
) - np.min(
    [
...
# 19
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV5 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-05"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 20
# df_sql['PRES MED VACUO - US8'] = np.mean([df_sql['PRES1_I@08FI-BV-827I'],
# df_sql['PRES1_I@08FI-BV-827I-01'], df_sql['PRES1_I@08FI-BV-827I-02'],
# df_sql['PRES1_I@08FI-BV-827I-03'], df_sql['PRES1_I@08FI-BV-827I-04'],
# df_sql['PRES1_I@08FI-BV-827I-05R'], df_sql['PRES1_I@08FI-BV-827I-06'],
# df_sql['PRES1_I@08FI-BV-827I-07'], df_sql['PRES1_I@08FI-BV-827I-08'],
# df_sql['PRES1_I@08FI-BV-827I-09'], df_sql['PRES1_I@08FI-BV-827I-10R']],
# axis=0)

# 21
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV9 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-09"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 22
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV7 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-07"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 23
df_sql["GANHO PRENSA - US8"] = (
    df_sql["SUP_SE_PP_L@08PR"] - df_sql["SUP_SE_PR_L@08FI"]
)

# 24
# df_sql['SP TEMP GQ-MED GQ3-16 - US8'] = np.mean([df_sql[
# 'TEMP1_S@08QU-QU-855I-GQ03'], df_sql['TEMP1_S@08QU-QU-855I-GQ04'], df_sql[
# 'TEMP1_S@08QU-QU-855I-GQ05'], df_sql['TEMP1_S@08QU-QU-855I-GQ06'], df_sql[
# 'TEMP1_S@08QU-QU-855I-GQ07'], df_sql['TEMP1_S@08QU-QU-855I-GQ08'], df_sql[
# 'TEMP1_S@08QU-QU-855I-GQ09'], df_sql['TEMP1_S@08QU-QU-855I-GQ10'], df_sql[
# 'TEMP1_S@08QU-QU-855I-GQ11'], df_sql['TEMP1_S@08QU-QU-855I-GQ12'], df_sql[
# 'TEMP1_S@08QU-QU-855I-GQ13'], df_sql['TEMP1_S@08QU-QU-855I-GQ14'], df_sql[
# 'TEMP1_S@08QU-QU-855I-GQ16']],  axis=0)

# 25
df_sql["PERMEABILIDADE CV31 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-31"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 26
# p = df_sql['PESO1_I@08MO-BW-821I-01M1']>0
# df_sql['Desgaste de Corpo Moedor ton/h'] = df_sql[
# 'PESO1_I@08MO-BL-821I-01-MO01'][p]/df_sql['PESO1_I@08MO-BW-821I-01M1'][p]
...
df_sql["Escorregamento"] = (
    1000
    * df_sql["ROTA1_I@08PR-RP-822I-01M1"][p1 & p2]
    / df_sql["PESO1_I@08PR-BW-822I-01M1"][p1 & p2]
    / df_sql["PRES3_I@08PR-RP-822I-01"][p1 & p2]
)

# 28
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV8 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-08"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 29
p = df_sql["Vel Periferica R Fixo [m/s]"] > 0
df_sql["GAP CALC PRENSA - US8"] = (
    df_sql["PESO1_I@08PR-BW-822I-01M1"][p]
    / (1.5 * 3.5 * df_sql["Vel Periferica R Fixo [m/s]"][p])
) / 3.6

# 30
p = df_sql["PESO1_I@08PR-BW-822I-01M1"] > 0
# df_sql['CONS EE PRENSA - US8'] = (df_sql['POTE1_I@08PR-RP-822I-01M1'][p] +
# df_sql['POTE1_I@08PR-RP-822I-01M2'][p])/df_sql['PESO1_I@08PR-BW-822I-01M1'][p]
df_sql["CONS EE PRENSA - US8"] = df_sql["CONS1_Y@08PR-RP-822I-01"]

# 31
# df_sql['Tempo de mistura 1'] = df_sql['NIVE1_I@08MI-MI-832I-01'] / 100 * 7
# / (( df_sql['PESO1_I@08MI-BW-832I-01M1'] + df_sql[
# 'PESO1_I@08MI-LW-832I-01M1']) / 1.9)*3600

# 32
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV16 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-16"] / df_sql["PROD_PQ_Y@08US"][p]
)

# 33
# df_sql['MED - SP ALTURA CAMADA'] = np.mean([df_sql[
# 'NIVE1_C@08QU-FR-851I-01M1'], df_sql['NIVE2_I@08QU-FR-851I-01M1'], df_sql[
# 'NIVE3_I@08QU-FR-851I-01M1'], df_sql['NIVE4_I@08QU-FR-851I-01M1'], df_sql[
# 'NIVE5_I@08QU-FR-851I-01M1'], df_sql['NIVE6_I@08QU-FR-851I-01M1']],
# axis=0)-df_sql['NIVE1_S@08QU-FR-851I-01M1']

# 34
# df_sql['Tempo de mistura 2'] = df_sql['NIVE1_I@08MI-MI-832I-02'] / 100*7 /
# ((df_sql['PESO1_I@08MI-BW-832I-02M1'] + df_sql[
# 'PESO1_I@08MI-LW-832I-01M1']) / 1.9)*3600

# 35
df_sql["POT TOTAL VENT - US8"] = (
    df_sql["POTE1_I@08QU-PF-852I-01M1"]
    + df_sql["POTE1_I@08QU-PF-852I-02M1"]
    + df_sql["POTE1_I@08QU-PF-852I-03M1"]
    + df_sql["POTE1_I@08QU-PF-852I-04M1"]
    + df_sql["POTE1_I@08QU-PF-852I-05M1"]
    + df_sql["POTE1_I@08QU-PF-852I-06M1"]
)

# 56
df_sql["SOMA POTENCIA BV - US8"] = (
    df_sql["POTE1_I@08FI-BV-827I-01M1"]
    + df_sql["POTE1_I@08FI-BV-827I-02M1"]
    + df_sql["POTE1_I@08FI-BV-827I-03M1"]
    + df_sql["POTE1_I@08FI-BV-827I-04M1"]
    + df_sql["POTE1_I@08FI-BV-827I-05RM1"]
    + df_sql["POTE1_I@08FI-BV-827I-06M1"]
    + df_sql["POTE1_I@08FI-BV-827I-07M1"]
    + df_sql["POTE1_I@08FI-BV-827I-08M1"]
    + df_sql["POTE1_I@08FI-BV-827I-09M1"]
    + df_sql["POTE1_I@08FI-BV-827I-10RM1"]
)

# 36
p = df_sql["PROD FILTR - US8"] > 0
# df_sql['Calculo da Energia da Filtragem'] = df_sql['SOMA POTENCIA BV -
# US8'][p]/df_sql['PROD FILTR - US8'][p]
df_sql["Calculo da Energia da Filtragem"] = df_sql["CONS1_Y@08FI-BV-827I"]

# 37
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV4 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-04"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 38
# p=df_sql['PESO1_I@08MO-BW-821I-01M1']>0.1
# df_sql['Consumo de Energia (base minério úmido) kWh/ton 1'] = (df_sql[
# 'POTE1_I@08MO-MO-821I-01M1'][p]/df_sql['PESO1_I@08MO-BW-821I-01M1'][p])*1000
df_sql["Consumo de Energia (base minério úmido) kWh/ton"] = df_sql[
    "CONS1_Y@08MO-MOAG"
]

# 39
df_sql["RPM MED FILTROS - US8"] = np.mean(
    [
        df_sql["ROTA1_I@08FI-FL-827I-01M1"],
        df_sql["ROTA1_I@08FI-FL-827I-02M1"],
        df_sql["ROTA1_I@08FI-FL-827I-03M1"],
        df_sql["ROTA1_I@08FI-FL-827I-04M1"],
        df_sql["ROTA1_I@08FI-FL-827I-05RM1"],
        df_sql["ROTA1_I@08FI-FL-827I-06M1"],
        df_sql["ROTA1_I@08FI-FL-827I-07M1"],
        df_sql["ROTA1_I@08FI-FL-827I-08M1"],
        df_sql["ROTA1_I@08FI-FL-827I-09M1"],
        df_sql["ROTA1_I@08FI-FL-827I-10RM1"],
    ],
    axis=0,
)
...
# 40
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV6 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-06"][p] / df_sql["PROD_PQ_Y@08US"][p]
)
...
df_sql["Produtividade Pelotamento Virtual"] = (
    df_sql["PROD_PC_I@08US"][p] - df_sql["PESO1_I@08PE-TR-851I-03M1"][p]
) / ((7.5 / 2) * math.pi * df_sql["SOMA FUNC " "FILTROS"][p])

# 60
p = df_sql["SOMA FUNC FILTROS"] > 0
df_sql["Produtividade Pel Efetiva"] = (df_sql["PROD_PQ_Y@08US"][p]) / (
    ((7.5 / 2) * math.pi) * df_sql["SOMA FUNC FILTROS"][p]
)

# 41
df_sql["10 - DIF PRODUTIVI EFETIVA - VIRTUAL - CALC - US8"] = (
...
# 42
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV10 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-10"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 43
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV20 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-20"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 44
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV14 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-14"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 45
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV21 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-21"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 46
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV17 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-17"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 47
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV11 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-11"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 50
# df_sql['MV TEMP GQ-MED - US8'] = np.mean([df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ01'], df_sql['TEMP1_O@08QU-QU-855I-GQ02'], df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ03'],df_sql['TEMP1_O@08QU-QU-855I-GQ04'],df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ05'],df_sql['TEMP1_O@08QU-QU-855I-GQ06'],df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ07'],df_sql['TEMP1_O@08QU-QU-855I-GQ08'],df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ09'],df_sql['TEMP1_O@08QU-QU-855I-GQ10'],df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ11'],df_sql['TEMP1_O@08QU-QU-855I-GQ12'],df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ13'],df_sql['TEMP1_O@08QU-QU-855I-GQ14'],df_sql[
# 'TEMP1_O@08QU-QU-855I-GQ15'],df_sql['TEMP1_O@08QU-QU-855I-GQ16']], axis=0)

# 51
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["CONS ESPEC EE VENT - US8"] = (
    df_sql["POT TOTAL VENT - US8"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 52
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV1 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-01"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 53
df_sql["MAIOR TEMP PQ - US8"] = np.max(
    [df_sql["TEMP1_I@08PN-TR-860I-01"], df_sql["TEMP1_I@08PN-TR-860I-02"]],
    axis=0,
)

# 54
# p=df_sql['PESO1_I@08MO-BW-821I-02M1']>0.1
# df_sql['Consumo de Energia (base minério úmido) kWh/ton 2'] = (df_sql[
# 'POTE1_I@08MO-MO-821I-02M1'][p]/df_sql['PESO1_I@08MO-BW-821I-02M1'][p])*1000

# 55
p = df_sql["PROD_PQ_Y@08US"] > 0
df_sql["PERMEABILIDADE CV13 - CALC - US8"] = (
    df_sql["PRES1_I@08QU-WB-851I-13"][p] / df_sql["PROD_PQ_Y@08US"][p]
)

# 58
df_sql["vazao_antracito"] = (
    df_sql["PESO1_I@08MO-BW-813I-03M1"] + df_sql["PESO1_I@08MO-BW-813I-04M1"]
)

# 61
df_sql["%CF. ANT"] = (
    df_sql["QUIM_CFIX_PP_L@08PR"] / df_sql["QUIM_CARVAO_PP_L@08PR"]
)
...
df_sql["=PQ*24/768/FUNC"] = (
    df_sql["PROD_PQ_Y@08US"][p]
    * 24
    / 768
    / df_sql["FUNC1_D@08QU-FR-851I-01M1"][p]
)

# 64 -- Demanda Aileen - SE PP
df_sql["media vel de disco de pel"] = (
    (df_sql["ROTA1_I@08PE-BD-840I-01M1"] * df_sql["FUNC1_D@08PE-BD-840I-01M1"])
    + (
        df_sql["ROTA1_I@08PE-BD-840I-02M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-02M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-03M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-03M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-04M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-04M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-05M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-05M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-06M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-06M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-07M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-07M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-08M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-08M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-09M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-09M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-10M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-10M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-11M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-11M1"]
    )
    + (
        df_sql["ROTA1_I@08PE-BD-840I-12M1"]
        * df_sql["FUNC1_D@08PE-BD-840I-12M1"]
    )
) / (
    df_sql["FUNC1_D@08PE-BD-840I-01M1"]
    + df_sql["FUNC1_D@08PE-BD-840I-02M1"]
    + df_sql["FUNC1_D@08PE-BD-840I-03M1"]
    + df_sql["FUNC1_D@08PE-BD-840I-04M1"]
    + df_sql["FUNC1_D@08PE-BD-840I-05M1"]
...
df_sql["media rolo fixo prensa"] = np.mean(
    [df_sql["POTE1_I@08PR-RP-822I-01M1"], df_sql["POTE1_I@08PR-RP-822I-01M2"]],
    axis=0,
)
...
df_sql["media oleo prensa"] = np.mean(
    [df_sql["PRES2_I@08PR-RP-822I-01"], df_sql["PRES3_I@08PR-RP-822I-01"]],
    axis=0,
)
# df_sql['diferença oleo prensa'] = np.diff(df_sql[
# 'PRES2_I@08PR-RP-822I-01'],df_sql['PRES3_I@08PR-RP-822I-01']).(periods=1,
# axis=1)
# _, df_sql['diferenca oleo prensa'] = (df_sql[['PRES2_I@08PR-RP-822I-01',
# 'PRES3_I@08PR-RP-822I-01']]).diff(axis=1)
...
df_sql["soma balanca minerio misturador"] = (
    df_sql["PESO1_I@08MI-BW-832I-01M1"] + df_sql["PESO1_I@08MI-BW-832I-02M1"]
)

# 70 - Demanda Aileen - Modelo Queima, abrasao=compr
df_sql["soma balanca bentonita misturador"] = (
    df_sql["PESO1_I@08MI-LW-832I-01M1"] + df_sql["PESO1_I@08MI-LW-832I-02M1"]
)

# 71 - Demanda Aileen - Modelo Queima, abrasao=compr
df_sql["soma balanca retorno correia"] = (
    df_sql["PESO1_I@08PE-TR-840I-28M1"] + df_sql["PESO1_I@08PE-TR-840I-29M1"]
)
...
df_sql["media disco de pelotamento"] = np.mean(
    [
        df_sql["ROTA1_I@08PE-BD-840I-01M1"],
        df_sql["ROTA1_I@08PE-BD-840I-02M1"],
        df_sql["ROTA1_I@08PE-BD-840I-03M1"],
        df_sql["ROTA1_I@08PE-BD-840I-04M1"],
        df_sql["ROTA1_I@08PE-BD-840I-05M1"],
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ01'] = df_sql['TEMP1_I@08QU-QU-855I-GQ01'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ02'] = df_sql['TEMP1_I@08QU-QU-855I-GQ02'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ03'] = df_sql['TEMP1_I@08QU-QU-855I-GQ03'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ04'] = df_sql['TEMP1_I@08QU-QU-855I-GQ04'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ05'] = df_sql['TEMP1_I@08QU-QU-855I-GQ05'][
# p][p1][p2][p3]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ06'] = df_sql['TEMP1_I@08QU-QU-855I-GQ06'][
# p][p1][p2][p3]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ07'] = df_sql['TEMP1_I@08QU-QU-855I-GQ07'][
# p][p1][p2][p3]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ08'] = df_sql['TEMP1_I@08QU-QU-855I-GQ08'][
# p][p1][p2][p3]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ09'] = df_sql['TEMP1_I@08QU-QU-855I-GQ09'][
# p][p1][p2][p3]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ10'] = df_sql['TEMP1_I@08QU-QU-855I-GQ10'][
# p][p1][p2][p3]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ11'] = df_sql['TEMP1_I@08QU-QU-855I-GQ11'][
# p][p1][p2][p3]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ12'] = df_sql['TEMP1_I@08QU-QU-855I-GQ12'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ13'] = df_sql['TEMP1_I@08QU-QU-855I-GQ13'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ14'] = df_sql['TEMP1_I@08QU-QU-855I-GQ14'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ15'] = df_sql['TEMP1_I@08QU-QU-855I-GQ15'][
# p][p1]
...
# df_sql['TEMP1_I@08QU-QU-855I-GQ16'] = df_sql['TEMP1_I@08QU-QU-855I-GQ16'][
# p][p1]

# # 90
# #Filtro funcionamento do queimador A -- solicitar tag func, Aileen não
# encontrou
# p=df_sql['']>0
# df_sql['TEMP1_I@08QU-QU-855I-GQA'] = df_sql['TEMP1_I@08QU-QU-855I-GQA'][p][p1]

# # 91
# #Filtro funcionamento do queimador B -- solicitar tag func, Aileen não
# encontrou
# p=df_sql['']>0
# df_sql['TEMP1_I@08QU-QU-855I-GQA'] = df_sql['TEMP1_I@08QU-QU-855I-GQA'][p][p1]

# # 92
# #Filtro funcionamento do queimador C -- solicitar tag func, Aileen não
# encontrou
# p=df_sql['']>0
...
df_sql["calc_distribuicao_gran"] = (
    df_sql["GRAN_12,5_PQ_L@08QU"]
    + df_sql["GRAN_16_PQ_L@08QU"]
    + df_sql["GRAN_10_PQ_L@08QU"]
    + df_sql["GRAN_8_PQ_L@08QU"]
)

# 94
# modelo de densidade moinho
# adicionar, *bomba de retorno do tanque/(bomba retorno do tanque+vazão de
# saída do tanque) -- verificar correlação com possibilidade
df_sql["bomba de retorno tanque"] = df_sql["VAZA1_I@08MO-BP-821I-01"] / (
    df_sql["VAZA1_I@08MO-BP-821I-01"] + df_sql["VAZA1_I@08MO-BP-821I-01M1"]
)
...
df_sql["media de densidade"] = (
    (df_sql["DENS1_C@08HO-BP-826I-05"] * df_sql["FUNC1_D@08HO-BP-826I-05M1"])
    + (
        df_sql["DENS1_C@08HO-BP-826I-06R"]
        * df_sql["FUNC1_D@08HO-BP-826I-06RM1"]
    )
    + (df_sql["DENS1_C@08HO-BP-826I-07"] * df_sql["FUNC1_D@08HO-BP-826I-07M1"])
    + (
        df_sql["DENS1_C@08HO-BP-826I-08R"]
        * df_sql["FUNC1_D@08HO-BP-826I-08RM1"]
    )
) / (
    df_sql["FUNC1_D@08HO-BP-826I-05M1"]
    + df_sql["FUNC1_D@08HO-BP-826I-06RM1"]
    + df_sql["FUNC1_D@08HO-BP-826I-07M1"]
    + df_sql["FUNC1_D@08HO-BP-826I-08RM1"]
)
...
df_sql["media alimentacao do disco"] = np.mean(
    [
        df_sql["PESO1_I@08PE-BW-840I-01M1"],
        df_sql["PESO1_I@08PE-BW-840I-02M1"],
        df_sql["PESO1_I@08PE-BW-840I-03M1"],
        df_sql["PESO1_I@08PE-BW-840I-04M1"],
        df_sql["PESO1_I@08PE-BW-840I-05M1"],
        df_sql["PESO1_I@08PE-BW-840I-06M1"],
        df_sql["PESO1_I@08PE-BW-840I-07M1"],
        df_sql["PESO1_I@08PE-BW-840I-08M1"],
        df_sql["PESO1_I@08PE-BW-840I-09M1"],
        df_sql["PESO1_I@08PE-BW-840I-10M1"],
        df_sql["PESO1_I@08PE-BW-840I-11M1"],
        df_sql["PESO1_I@08PE-BW-840I-12M1"],
    ],
    axis=0,
)
...
df_sql["media tm"] = np.mean(
    [
        df_sql["GRAN_OCS_TM@08PE-BD-840I-01"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-02"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-03"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-04"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-05"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-06"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-07"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-08"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-09"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-10"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-11"],
        df_sql["GRAN_OCS_TM@08PE-BD-840I-12"],
    ],
    axis=0,
)
...
df_sql["media_potencia_moinho"] = np.mean(
    [
        df_sql["POTE1_I@08MO-MO-821I-01M1"],
        df_sql["POTE1_I@08MO-MO-821I-02M1"],
        df_sql["POTE1_I@08MO-MO-821I-03M1"],
    ],
    axis=0,
)
...
#     ('corpo_moedor_especifico_1','PESO2_Q@08MO-TR-821I-02M1-MO01',
#     'PESO1_I@08MO-BW-821I-01M1'),
#     ('corpo_moedor_especifico_2','PESO2_Q@08MO-TR-821I-02M1-MO02',
#     'PESO1_I@08MO-BW-821I-02M1'),
#     ('corpo_moedor_especifico_3','PESO2_Q@08MO-TR-821I-02M1-MO03',
#     'PESO1_I@08MO-BW-821I-03M1'),
# ]:
#     p = df_sql[c_producao]>0
#     df_sql[c_corpo_moedor_especifico] = 0
#     df_sql.loc[p, c_corpo_moedor_especifico] = df_sql[p][c_corpo_moedor] /
#     df_sql[p][c_producao]
df_sql["corpo_moedor_especifico"] = (
    (
        df_sql["PESO1_Q@08MO-TR-821I-02M1-MO01"]
        + df_sql["PESO1_Q@08MO-TR-821I-02M1-MO02"]
        + df_sql["PESO1_Q@08MO-TR-821I-02M1-MO03"]
    )
    * df_sql["FUNC1_D@08QU-FR-851I-01M1"]
    * 1000
    / df_sql["PROD_PQ_Y@08QU-ACUM"]
)

# df_sql['corpo_moedor_especifico'] = (
#     ((df_sql['PESO1_Q@08MO-TR-821I-02M1-MO01']*df_sql[
#     'FUNC1_D@08QU-FR-851I-01M1'])
#     + (df_sql['PESO1_Q@08MO-TR-821I-02M1-MO02']*df_sql[
#     'FUNC1_D@08QU-FR-851I-01M1'])
#     + (df_sql['PESO1_Q@08MO-TR-821I-02M1-MO03']*df_sql[
#     'FUNC1_D@08QU-FR-851I-01M1'])) * 1000
#     / df_sql['PROD_PQ_Y@08QU-ACUM'])
...
# df_sql['antracito'] = df_sql['PESO1_I@08MO-BW-813I-03M1']+df_sql[
...
# df_sql = df_sql.drop(['PESO1_I@08MO-BW-813I-03M1','PESO1_I@08MO-BW-813I
# -04M1'], axis=1)
# df_sql = df_sql.drop(['PESO1_I@08MO-BW-813I-03M1',
# 'PESO1_I@08MO-BW-813I-04M1'], axis=1)
...
# df_otm_forno['lado'] = -1
df_sql["Media_temp"] = df_sql[
    ["TEMP1_I@08PN-TR-860I-01", "TEMP1_I@08PN-TR-860I-02"]
].mean(1)
...
def df_otm_forno_filtro(x):
    if (
        x["POSI1_I@08QU-GH-851I-41M1"] >= 10
        and x["POSI1_I@08QU-GH-851I-42M1"] >= 10
    ):
        #         x['lado'] = 2

        x["Media_temp"] = (
            x["TEMP1_I@08PN-TR-860I-01"] + x["TEMP1_I@08PN-TR-860I-02"]
        ) / 2
    #         x['Media_posi'] = (x['POSI1_I@08QU-GH-851I-41M1'] + x[
    #         'POSI1_I@08QU-GH-851I-42M1'])/2
...
## HOT FIX
# df_sql['VAZA1_I@08QU-ST-855I-01'] = df_sql['VAZA1_I@08QU-ST-855I-01'] /
# df_sql['PROD_PQ_Y@08US'][df_sql['PROD_PQ_Y@08US'] > 0]

# df_sql['bentonita'] = (df_sql['PESO1_I@08MI-LW-832I-01M1'] + df_sql[
# 'PESO1_I@08MI-LW-832I-02M1'])/(df_sql['PESO1_I@08MI-BW-832I-01M1'] +
# df_sql['PESO1_I@08MI-BW-832I-02M1']) * 100 / 0.8693 / df_sql['PROD_PQ_Y@08US']

# df_sql['bentonita'] = (df_sql['PESO1_I@08MI-LW-832I-01M1'] + df_sql[
# 'PESO1_I@08MI-LW-832I-02M1'])/ df_sql['PROD_PQ_Y@08US'] * 1000

df_sql["bentonita"] = (
    (df_sql["PESO1_I@08MI-LW-832I-01M1"] + df_sql["PESO1_I@08MI-LW-832I-02M1"])
    / (
        df_sql["PESO1_I@08MI-BW-832I-01M1"]
        + df_sql["PESO1_I@08MI-BW-832I-02M1"]
    )
    * df_sql["FUNC1_D@08QU-FR-851I-01M1"]
    * 1000
)
...
# df_sql['FUNC1_Q@08FI-BV-827I-01M1','FUNC1_Q@08FI-BV-827I-02M1',
# 'FUNC1_Q@08FI-BV-827I-03M1','FUNC1_Q@08FI-BV-827I-04M1',
# 'FUNC1_Q@08FI-BV-827I-05RM1','FUNC1_Q@08FI-BV-827I-06M1',
# 'FUNC1_Q@08FI-BV-827I-07M1','FUNC1_Q@08FI-BV-827I-08M1',
# 'FUNC1_Q@08FI-BV-827I-09M1','FUNC1_Q@08FI-BV-827I-10RM1']
...
temp["prod_seca"] = (
    df_sql["PESO1_I@08FI-TR-827I-01M1"] + df_sql["PESO1_I@08FI-TR-827I-02M1"]
) * (1 - df_sql["UMID_H2O_PR_L@08FI"] / 100)

df_sql["Consumo_energia_especifica"] = (
...
df_sql.loc[
    df_sql["Consumo_energia_especifica"] > 3.12, "Consumo_energia_especifica"
] = np.nan
...
# datasets_limits_path = 'abfss://insight@usazu1valesa001.dfs.core.windows
# .net/Workarea/Pelletizing/Process_Optimization/Usina08/Otimizacao/files/'
...
limits["04 - MOAGEM - SPSS - US8 - PATRICIA"][
    "PESO1_I@08MO-BW-821I-02M1_DP"
] = (0, None)
limits["04 - MOAGEM - SPSS - US8 - PATRICIA"][
    "PESO1_I@08MO-BW-821I-03M1_DP"
] = (0, None)
...
limits["11 - TRATAMENTO TERMICO - SPSS - US8"]["TORQ1_I@08QU-PF-852I-02M1"] = (
    0,
    None,
)
limits["11 - TRATAMENTO TERMICO - SPSS - US8"]["TORQ1_I@08QU-PF-852I-04M1"] = (
    0,
    None,
)
limits["11 - TRATAMENTO TERMICO - SPSS - US8"]["TORQ1_I@08QU-PF-852I-05M1"] = (
    0,
    None,
)
limits["11 - TRATAMENTO TERMICO - SPSS - US8"]["TORQ1_I@08QU-PF-852I-08M1"] = (
    0,
    None,
)
limits["11 - TRATAMENTO TERMICO - SPSS - US8"]["PESO1_I@08PN-TR-860I-09M1"] = (
    0,
    None,
)
limits["11 - TRATAMENTO TERMICO - SPSS - US8"]["VELO2_C@08QU-FR-851I-01M1"] = (
    0,
    None,
)
...
# for t in dfs['10 - AGLOMERACAO - SPSS - US8'][[x for x in datasets[
# 'distribuicao gran'].columns if 'GRAN_OCS_TM' in x]]:
#   if 'Y' not in t:
#     limits['10 - AGLOMERACAO - SPSS - US8'][t] = (dfs['10 - AGLOMERACAO -
#     SPSS - US8'][t].quantile(0.15), None)
...
volt_exclude.extend(
    ["VOLT2_I@08QU-PP-871I-01-PAT0" + str(i) for i in range(1, 7)]
)
# se os limites forem aplicados estas tags volt ficam impossibilitadas de
# serem desligadas na otimizacao (min > 0)
...
for k, df in dfs.items():
    for t in df.columns:
        if (
            (t in limits[k])
            and (limits[k][t] is not None)
            and (t not in limits_exclude)
        ):
...
def find_col(dict_object, col_name):
    ...
    valor : pandas.DataFrame
        variáveis do modelo onde cada linha equivale a 1 hora de captura
        dos dados
    """
...
for base in datasets_confs:
        ...
        if len(tags) == 0:
            if k == "10 - AGLOMERACAO - SPSS - US8":
                dfs_variaveis.append(
                    dfs[k].drop(["PROD_PQ_Y@08US", "PROD_PC_I@08US"], axis=1)
                )
            else:
    ...
    for qual, conf in base["targets"].items():
        ...
        if len(set(base["status"]) - set(base_status)) > 0:
            log.info(
                f'{set(base["status"]) - set(base_status)}, \t, '
                f"NAO ENCONTRADOS"
            )
        ...
        if qual in ["GRAN PR", "SE PR", "SE PP"] + [
            "abrasao",
            "compressao",
            "basicidade",
            "finos",
            ...
            for suf in ["CA", "CN", "CE", "BR"]:
                ...
                if len(cs) > 0:
                    datasets[qual]["minerio_" + suf] = datasets[qual][cs].sum(
                        axis=1
                    )
        ...
        if qual in ["SE PP"]:
            ...
            # ts_torque = ['TORQ2_I@08PR-RP-822I-01M1',
        ...
        if qual == "basicidade":
            ...
            removerTAGs = [
                x
                for x in datasets[qual].columns
                if x
                in [
                    "QUIM_CAO_PP_L@08PR",
                    "P DP",
        ...
        if qual != "basicidade":
            ...
            if "para_remover" in base:
                #                 for t_para_remover in base['para_remover']:
                #                 assert t_para_remover in datasets[
                #                 qual].columns, (qual, t_para_remover)
                removerTAGs = [
                    x
                    for x in datasets[qual].columns
                    if x in base["para_remover"]
                ]
                datasets[qual] = datasets[qual].drop(removerTAGs, axis=1)
        ...
        datasets[
            qual
        ].index.name = "Processo"  # tava dando erro na 'distribuicao
        # gran' apos o dropna()

# solucao temporaria, nao entendi de onde vem essa coluna. Ela surge em
# distribuicao gran como 0, depois renomeado para vazio
...
df_sql["rel_gran"] = (
    (
        (df_sql["GRAN_12,5_PQ_L@08QU"] + df_sql["GRAN_16_PQ_L@08QU"])
        / (df_sql["GRAN_10_PQ_L@08QU"] + df_sql["GRAN_8_PQ_L@08QU"])
    )
    .fillna(method="bfill")
    .fillna(method="ffill")
    .fillna(0)
...
datasets["relacao gran"] = (
    datasets["relacao gran"].join(df_sql["rel_gran"]).copy()
)
datasets["relacao gran"] = datasets["relacao gran"][
    (df_sql["rel_gran"] >= 0.8) & (df_sql["rel_gran"] <= 1.2)
]

for k, v in datasets.items():
    if ("status" in datasets[k].columns) and (
        ((datasets[k]["status"] == 0).sum() == datasets[k].shape[0])
    ):
        log.info(k)
        datasets[k]["status"] = 1
...
for k, df in datasets_originais.items():
    ...
    if n_coluns != datasets_originais[k].shape[1]:
        log.info(k)
...
# cv_size = 0.3 # em relação ao tamanho do treino, sem considerar o test
# cv_n = 3
...
log.debug(
    "recupera datasets, mas datasets_originais ja era cópia de datasets!!!!"
)
...
for i in range(1, 13):
    ...
    datasets["rota_disco_" + str(i)] = datasets["finos"][
        [v_peso, v_tm, v_func, v_rota]
    ].rename(columns={v_func: "status"})
...
# target
datasets["dens_moinho_1"]["DENS1_C@08MO-MO-821I-01"] = datasets["SE PR"][
    "DENS1_C@08MO-MO-821I-01"
].copy()

# retirando func -> transformando ele em status para ser dropado no treino (
# teoricamente, pelo datasets_confs, ele já é o status, mas, quando fui olhar
# o describe, estava um pouco diferente. por isso to fazendo essa substituição)
datasets["dens_moinho_1"]["status"] = datasets["dens_moinho_1"][
    "FUNC1_D@08MO-MO-821I-01M1"
].copy()
# datasets['dens_moinho_1'] = datasets['dens_moinho_1'][datasets[
# 'dens_moinho_1']['FUNC1_D@08MO-MO-821I-01M1'] == 1].copy()
datasets["dens_moinho_1"].drop(
    columns=["FUNC1_D@08MO-MO-821I-01M1"], inplace=True
)

# criando dataset
datasets["dens_moinho_2"] = (
    datasets["SE PR"]
    .drop(
        columns=[
            "DENS1_C@08MO-MO-821I-01",
...
# target
datasets["dens_moinho_2"]["DENS1_C@08MO-MO-821I-02"] = datasets["SE PR"][
    "DENS1_C@08MO-MO-821I-02"
].copy()
...
# teoricamente, pelo datasets_confs, ele já é o status, mas, quando fui olhar
# o describe, estava um pouco diferente. por isso to fazendo essa substituição)
datasets["dens_moinho_2"]["status"] = datasets["dens_moinho_2"][
    "FUNC1_D@08MO-MO-821I-02M1"
].copy()
# datasets['dens_moinho_2'] = datasets['dens_moinho_2'][datasets[
# 'dens_moinho_2']['FUNC1_D@08MO-MO-821I-02M1'] == 1].copy()
datasets["dens_moinho_2"].drop(
    columns=["FUNC1_D@08MO-MO-821I-02M1"], inplace=True
)

# criando dataset
datasets["dens_moinho_3"] = (
    datasets["SE PR"]
    .drop(
        columns=[
            "DENS1_C@08MO-MO-821I-01",
...
# target
datasets["dens_moinho_3"]["DENS1_C@08MO-MO-821I-03"] = datasets["SE PR"][
    "DENS1_C@08MO-MO-821I-03"
].copy()
...
# teoricamente, pelo datasets_confs, ele já é o status, mas, quando fui olhar
# o describe, estava um pouco diferente. por isso to fazendo essa substituição)
datasets["dens_moinho_3"]["status"] = datasets["dens_moinho_3"][
    "FUNC1_D@08MO-MO-821I-03M1"
].copy()
# datasets['dens_moinho_3'] = datasets['dens_moinho_3'][datasets[
# 'dens_moinho_3']['FUNC1_D@08MO-MO-821I-03M1'] == 1].copy()
datasets["dens_moinho_3"].drop(
    columns=["FUNC1_D@08MO-MO-821I-03M1"], inplace=True
)

# tags ainda não estão no datalake - pedido da aileen
# add_model_tags(datasets, df_sql, 'dens_moinho_1',
# ['VAZA1_I@08MO-BP-821I-01M1', 'VAZA1_I@08AP-BP-875I-01',
# 'DENS1_I@08AP-TQ-875I-03', 'VAZA1_I@08MO-BP-821I-01',
# 'VAZA1_I@08MO-MO-821I-01', 'VAZA1_I@08MO-TQ-821I-01', 'UMID_H2O_AM_L@08MI'])
# add_model_tags(datasets, df_sql, 'dens_moinho_2',
# ['VAZA1_I@08MO-BP-821I-02M1', 'VAZA1_I@08AP-BP-875I-01',
# 'DENS1_I@08AP-TQ-875I-03', 'VAZA1_I@08MO-BP-821I-02',
# 'VAZA1_I@08MO-MO-821I-02', 'VAZA1_I@08MO-TQ-821I-02'])
# add_model_tags(datasets, df_sql, 'dens_moinho_3',
# ['VAZA1_I@08MO-BP-821I-03M1', 'VAZA1_I@08AP-BP-875I-01',
# 'DENS1_I@08AP-TQ-875I-03', 'VAZA1_I@08MO-BP-821I-03',
# 'VAZA1_I@08MO-MO-821I-03', 'VAZA1_I@08MO-TQ-821I-03'])
...
add_model_tags(
    datasets, df_sql, "produtividade filtragem", ["UMID_H2O_PR_L@08FI"]
)

datasets_teste = {}
for k, v in datasets.items():
    log.info(k)
    datasets_teste[k] = v.rename(
        columns=lambda x: re.sub(r"^[\d.-]+\s*", "", str(x))
    )
...
tags_compressao_abrasao = [
    "- DIF PRODUTIVI EFETIVA - VIRTUAL - CALC - US8",
    "bentonita",
...
datasets["abrasao"].drop(
    columns=tags_compressao_abrasao, axis=1, inplace=True, errors="ignore"
)
...
# ['PEVT_PF852I05_M1_PIT_OCS_CDV_09a12', 'PRES1_C@08QU-PF-852I-04M1',
# 'PRES1_I@08QU-DU-853I-12', 'PRES2_C@08QU-PF-852I-03M1'] Não encontradas no
# dataset
...
# ['PRES1_C@08QU-PF-852I-04M1', 'PRES1_I@08QU-DU-853I-12',
# 'PRES2_C@08QU-PF-852I-03M1'] Não encontradas no dataset
...
# ['DESV1_I@08PA-TR-811I-08M1', 'DESV1_I@08MO-BW-813I-03M1',
# 'DESV1_I@08MO-BW-813I-04M1', 'PEVT_US8_MOA_CVMI_AN_AIT_LAB_P_FG_-0,045MM']
...
# *bomba de retorno do tanque/(bomba retorno do tanque+vazão de saída do
# tanque): verificar correlação para possibilidade
...
datasets["energia_forno"].drop(
    columns=["PROD_PQ_Y@08US", "status"], axis=1, inplace=True, errors="ignore"
)

# energia_moinho

datasets["energia_forno"].drop(
    columns=["ProducaoPQ_Moagem", "status"],
    axis=1,
    inplace=True,
    errors="ignore",
)
...
# #comentar	DENS1_C@08MO-MO-821I-01,DENS1_C@08MO-MO-821I-02,
# DENS1_C@08MO-MO-821I-03

# # "correlação com Energia elétrica Média" - POTE1_I@08MO-MO-821I-01M1,
# POTE1_I@08MO-MO-821I-02M1,POTE1_I@08MO-MO-821I-03M1

# datasets['GRAN PR'].drop(columns = ['antracito','CARV1_OCS_S@08PP',
# 'FUNC1_D@08AD-BR-813I-01M1',	'FUNC1_D@08AD-BR-813I-02M1',
# 'FUNC1_D@08MO-MO-821I-01M1',	'FUNC1_D@08MO-MO-821I-02M1',
# 'FUNC1_D@08MO-MO-821I-03M1',	'NIVE1_I@08MO-SI-813I-06',
# 'NIVE1_I@08MO-TQ-821I-01',	'NIVE1_I@08MO-TQ-821I-02',
# 'NIVE1_I@08MO-TQ-821I-03',	'PESO1_I@08MO-BW-813I-01M1',
# 'PESO1_I@08MO-BW-813I-03M1',	'PESO1_I@08MO-BW-813I-04M1',
# 'PESO2_I@08MO-BW-813I-03M1',	'PESO2_I@08MO-BW-813I-04M1',
# 'PESO2_Q@08MO-TR-821I-02M1-MO01',	'PESO2_Q@08MO-TR-821I-02M1-MO02',
# 'PESO2_Q@08MO-TR-821I-02M1-MO03',	'ROTA1_I@08AD-BR-813I-01M1',
# 'ROTA1_I@08AD-BR-813I-02M1', 'qtde_moinhos',
# 'PESO2_Q@08MO-TR-821I-02M1-MO01','PESO2_Q@08MO-TR-821I-02M1-MO02',
# 'PESO2_Q@08MO-TR-821I-02M1-MO03', 'calcario'], axis=1, inplace = True,
# errors="ignore")

# add_model_tags(datasets, df_sql, 'GRAN PR', ['media balanca moinho',
# 'media_vazao_antracito', 'media_potencia_moinho', 'bomba de retorno
# tanque', 'GRAN_-0,045_PR_L@08FI'])
...
# datasets['relacao gran'].drop(columns = gran_ocs_remover, axis=1, inplace =
# True,errors="ignore")
...
# # rota_disco_1
# add_model_tags(datasets, df_sql, 'rota_disco_1',
# ['ROTA1_I@08PE-PN-840I-01M1','POSI1_I@08PE-BD-840I-01',
# 'PESO1_I@08PE-TR-840I-30M1', 'PESO1_I@08PE-TR-851I-03M1',
# 'PESO1_I@08PN-TR-860I-09M1', 'UMID_H2O_PR_L@08FI', 'GRAN_-0,045_PP_L@08PR',
# 'SUP_SE_PP_L@08PR', 'POSI1_I@08PE-BD-840I-01', 'ROTA1_I@08PE-PN-840I-01M1',
...
modelos_remover = []
modelos_remover.extend(
    ["temp_precipitador_1", "temp_precipitador_2", "temp_precipitador_3"]
)
...
log.info(
    f"null_tags: {null_tags}, null_tags perc: {round(null_tags / teste.shape[0] * 100, 2)}"
)
log.info(
    f"zero_tags: {zero_tags}, zero_tags perc: {round(zero_tags / teste.shape[0] * 100, 2)}"
)
...
log.info(
    f"null_tags: {null_tags}, null_tags perc: {round(null_tags / teste_2021.shape[0] * 100, 2)}"
)
log.info(
    f"zero_tags: {zero_tags}, zero_tags perc: {round(zero_tags / teste_2021.shape[0] * 100, 2)}"
)
...
log.info(
    f"null_tags: {null_tags}, null_tags perc: {round(null_tags / teste_2022.shape[0] * 100, 2)}"
)
log.info(
    f"zero_tags: {zero_tags}, zero_tags perc: {round(zero_tags / teste_2022.shape[0] * 100, 2)}"
)
